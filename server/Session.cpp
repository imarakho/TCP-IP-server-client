...
void    Session::handle_read(const boost::system::error_code &error, size_t bytes_transferred)
{
    ...
    {
        std::string resp = "";
        block_number == 1 ? 0 : time(&res_time);
        ...
        Parser p(data_);
        if(p.output(resp))
            resp = "Message with len of " + std::to_string(bytes_transferred) + " bytes was received by client " + std::to_string(this->id) + ".\n";
        else
            resp = "\033[0;31mAVARIA";
...
void    Session::handle_write(const boost::system::error_code &error)
{
    if(!error)
    {
        block_number == 1 ? 0 : time(&req_time);
